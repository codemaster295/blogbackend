...
router.post('/createblog/uploadimage/multiple', async (req, res) => {
	upload(req, res, async function (err) {
		if (err instanceof multer.MulterError) {
			res.send(err)
		} else if (err) {
			...
			var promises = req.files.map((file) => {
				return storageRef.upload(file.path, {
					public: true,
					destination: `blogmmo/${file.filename}`,
					metadata: {
						firebaseStorageDownloadTokens: uuidv4(),
					}
				}).then((filedata) => {
					dataArr.push(filedata[0].metadata.mediaLink)
				})
			})
